...
import { Search, ShieldCheck, Video, Clock, Headphones, CheckCircle2 } from 'lucide-react';
...
export function Hero() {
  ...
  const onSearch = () => navigate(`/search${q ? `?q=${encodeURIComponent(q)}` : ''}`);

  return (
    <section className="container-app pt-6">
      <div
        className="relative flex gap-8 overflow-hidden rounded-card bg-hero-gradient px-6 py-10 shadow-soft sm:px-10 lg:gap-12 lg:py-14"
      >
        ...
        <div className="relative hidden w-[300px] shrink-0 lg:block xl:w-[380px]">
          <img
            ...
            alt=""
            className="absolute -top-10 left-0 h-[calc(100%+80px)] w-full rounded-2xl object-cover object-top"
          />
        </div>
        ...
        <div className="relative z-10 flex max-w-xl flex-1 flex-col justify-center gap-5 font-cairo">
          ...
          <div className="flex max-w-md">
            <div className="flex flex-1 items-center overflow-hidden rounded-btn bg-white/95">
              <button onClick={onSearch} className="px-4 text-primary" aria-label="بحث">
                <Search size={18} />
              </button>
              <input
                value={q}
          ...
          <div className="flex flex-wrap gap-4">
            {features.map((f, i) => {
              ...
              return (
                <span key={f} className="inline-flex items-center gap-1.5 text-xs text-white/90">
                  <Icon size={14} /> {f}
                </span>
              );
            })}
