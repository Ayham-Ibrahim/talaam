...
/* ---------- Pill ---------- */
export function Pill({ children, active = false, dot, className = '' }) {
  return (
    <span
      className={`inline-flex items-center gap-1.5 text-[13px] px-3.5 py-1.5 rounded-pill border transition-colors ${
        active ? 'bg-primary text-white border-primary' : 'bg-canvas text-ink-soft border-line'
      } ${className}`}
    >
...
export function Avatar({ src, name = '', size = 'md', className = '' }) {
  ...
  const initials = name.replace(/^[أا]\.\s*/, '').trim().charAt(0);
  return (
    <div
...
/* ---------- StarRating ---------- */
export function StarRating({ value = 0, showValue = true, size = 14, className = '' }) {
  return (
    <span className={`inline-flex items-center gap-1 ${className}`}>
      {showValue && <span className="text-sm font-semibold text-ink">{value}</span>}
...
/* ---------- PriceTag ---------- */
export function PriceTag({ amount, suffix = '', currency = 'USD', className = '' }) {
...
/* ---------- EmptyState ---------- */
export function EmptyState({ title = 'لا توجد نتائج', hint = '', action = null }) {
  return (
