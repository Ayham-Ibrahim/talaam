...
import { BookOpen, CalendarDays, Check, Clock, GraduationCap, Layers, Users } from 'lucide-react';
...
/** One info row — icon in a soft primary circle + label, same rhythm as the booking drawer's own rows */
function DetailRow({ icon: Icon, children }) {
  return (
    <div className="flex items-center gap-3 rounded-2xl border border-line p-3 text-sm text-ink">
...
function PackageDetailsDrawer({ pkg, onClose }) {
  const t = useT();
  const currency = useCurrencyStore((s) => s.currency);
  const isGroup = pkg?.sessionFormat === 'group';
  const dates = pkg && isGroup ? scheduleDates(pkg.schedules) : [];

  return (
    <Drawer open={Boolean(pkg)} onClose={onClose} title={pkg?.title}>
      {pkg && (
        <div className="flex flex-col gap-5 text-start">
          {pkg.subject && <p className="-mt-2 text-sm font-medium text-ink-soft">{pkg.subject}</p>}
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {pkg.curricula?.map((c) => (
              <span key={c} className="rounded-pill bg-canvas px-3 py-1.5 text-xs font-semibold text-ink-soft">
                {c}
              </span>
            ))}
          </div>
          ...
          <div className="flex flex-col gap-2.5">
            ...
            {pkg.stages?.map((s) => (
              <DetailRow key={s} icon={GraduationCap}>
                {s}
              </DetailRow>
            ))}
            {pkg.grades?.length > 0 && (
          ...
          {dates.length > 0 && (
            <div>
              <h4 className="mb-2 text-sm font-bold text-ink">{t('teacher.scheduleLabel')}</h4>
              <div className="flex flex-col gap-2">
                {dates.map((d) => (
                  <div key={d} className="flex items-center gap-3 rounded-2xl border border-line p-3 text-sm text-ink">
                    <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-primary-light text-primary">
          ...
          {pkg.description && (
            <p className="whitespace-pre-line text-sm leading-relaxed text-ink-soft">{pkg.description}</p>
          )}

          <div className="flex items-center justify-between rounded-2xl bg-primary-light px-4 py-3.5">
            <span className="text-sm font-bold text-ink">{t('booking.total')}</span>
            <span className="text-2xl font-bold text-primary">{priceLabel(pkg.price, currency)}</span>
          </div>
        </div>
      )}
    </Drawer>
...
export function PackagesSection({ packages, isLoading, isError, refetch, selectedPackageId, onSelect }) {
  ...
  return (
    <div className="mt-8">
      ...
      <PackageDetailsDrawer pkg={detailsPkg} onClose={() => setDetailsPkg(null)} />
    </div>
  );
}
