...
function mapPublicProfile(raw) {
  return {
    ...
    typeLabel: TEACHER_TYPE_LABELS[raw.teacher_type] ?? raw.teacher_type,
    bio: raw.bio,
    teachingPhilosophyQuote: raw.teaching_philosophy_quote,
    teachingPhilosophyText: raw.teaching_philosophy_text,
    ...
    // شارات إنجاز حقيقية يمنحها الأدمن (Badge/BadgeAward عبر PublicTeacherResource)
    badges: (raw.badges ?? []).map((b) => ({ id: b.award_id, code: b.code, label: b.name_ar, icon: b.icon })),
...
export const teacherService = {
  async getTeachers(filters = {}) {
    ...
    const params = {
      ...
      curriculum_id: filters.curriculum || undefined,
