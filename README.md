...
## 🌐 النشر على سيرفر (VPS + Nginx)

بعد `npm run build`، ارفع مجلد `dist/` إلى السيرفر واستخدم إعداد Nginx الجاهز في
[`deploy/nginx.conf`](deploy/nginx.conf) — يضبط:
- تخزين مؤقت (`Cache-Control`) طويل الأمد لملفات `dist/assets/*` (مُسمّاة بـ hash فتتغير تلقائيًا مع كل تعديل).
- تخزين مؤقت أقصر لملفات الصور الثابتة في `public/` (لأنها تحتفظ بنفس الاسم بين كل نشرة).
- عدم تخزين `index.html` مؤقتًا حتى لا يشير لملفات JS/CSS قديمة بعد نشرة جديدة.
- fallback لكل المسارات إلى `index.html` (مطلوب لـ React Router).

راجع التعليقات داخل الملف لتبديل `YOUR_DOMAIN` ومسار `root`.
